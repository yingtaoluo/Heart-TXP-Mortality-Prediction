...
def create_TXP_static_data():
    if analyze_full_data:
        df = pd.read_csv(f'{DATASETS}/transplant_data_full.csv', low_memory=True, encoding='utf-8')
        save_path = f'{DATASETS}/df_txp_full.csv'
    else:
        df = pd.read_csv(f'{DATASETS}/transplant_data.csv', low_memory=True, encoding='utf-8')
        save_path = f'{DATASETS}/df_txp.csv'
    ...
    df_dd = pd.read_csv(f'{DATA}/thoracic_data/deceased_donor_data.csv', low_memory=False, encoding='utf-8')
    ...
    # donor_df = pd.read_csv(f"{DATA}/deceased_donor_data.csv", low_memory=False, encoding='utf-8')
    #
    ...
    df = read_donornet_and_merge(df, f"{DATA}/donornet/donors.csv", 'donor_record_datetime')
    df = read_donornet_and_merge(df, f"{DATA}/donornet/DonorNet_SAS/abgs.csv", 'ABG_DT')
    df = read_donornet_and_merge(df, f"{DATA}/donornet/DonorNet_SAS/cbc.csv", 'CBC_DT')
    df = read_donornet_and_merge(df, f"{DATA}/donornet/DonorNet_SAS/labpanels.csv", 'LABS_DT')
    df = read_donornet_and_merge(df, f"{DATA}/donornet/DonorNet_SAS/labvalues.csv", 'LABVALUES_DT')
    df = remove_binary_indicators(df)
...
def process_txp_data(file_path, selected_variables, model):
    ...
    if not analyze_full_data:
        wl_last_path = f'{DATASETS}/wl_dynamic_last_values.csv'
    ...
    univariate_df.to_csv(f'{RESULTS}/univariate_results.csv')
    significant_uni_df.to_csv(f'{RESULTS}/significant_univariate_results.csv')
    print(x_data.shape, x_data.columns)

    y_data = y_data.loc[df.index].reset_index(drop=True)

    if analyze_full_data:
        x_data.to_csv(f'{DATASETS}/{{}}YEAR/TXP_data_{{}}_full.csv'.format(YEARS, model), index=False)
        y_data.to_csv(f'{DATASETS}/{{}}YEAR/TXP_label_{{}}_full.csv'.format(YEARS, model), index=False)
        ...
        x_data.to_csv(f'{DATASETS}/{{}}YEAR/TXP_data_{{}}_imputed_full.csv'.format(YEARS, model), index=False)

    else:
        x_data.to_csv(f'{DATASETS}/{{}}YEAR/TXP_data_{{}}.csv'.format(YEARS, model), index=False)
        y_data.to_csv(f'{DATASETS}/{{}}YEAR/TXP_label_{{}}.csv'.format(YEARS, model), index=False)

        x_data = impute_dataset_mcmc(x_data, numerical_cols, 0)
        # x_data = impute_dataset_knn(x_data, numerical_cols, 5)
        # x_data = impute_dataset_median(x_data, numerical_cols)
        x_data.to_csv(f'{DATASETS}/{{}}YEAR/TXP_data_{{}}_imputed.csv'.format(YEARS, model), index=False)
...
def read_donornet(file_path):
    ...
    plt.savefig(f'{IMAGES}/number_offers.png')
    plt.close()
    ...
    plt.ylabel('Frequency', fontsize=12)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.savefig(f'{IMAGES}/offer_time.png')
    plt.close()


def run_txp_data(model_choice):
    if analyze_full_data:
        data_path = f'{DATASETS}/df_txp_full.csv'
    else:
        data_path = f'{DATASETS}/df_txp.csv'
...
if __name__ == '__main__':
    # offer_data_path = f'{DATA}/donornet/offers.csv'
