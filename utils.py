...
def load_cols_and_rows(df, task_type='TXP'):
    if task_type == 'TXP':
        wl_codes = pd.read_csv(f'{CHECKPOINTS}/TXP_row_codes.csv')['WL_ID_CODE'].tolist()
        columns_to_drop = pd.read_csv(f'{CHECKPOINTS}/TXP_columns_to_drop.csv')['drop'].tolist()
    else:
        wl_codes = pd.read_csv(f'{CHECKPOINTS}/WL_row_codes.csv')['WL_ID_CODE'].tolist()
        columns_to_drop = pd.read_csv(f'{CHECKPOINTS}/WL_columns_to_drop.csv')['drop'].tolist()
...
def handle_missing_values(df, cat_cols, data_type, num_threshold, cat_threshold):
    ...
    if data_type == 'TXP':
        pd.DataFrame(columns_to_drop, columns=['drop']).to_csv(f'{CHECKPOINTS}/TXP_columns_to_drop.csv', index=False)
        df['WL_ID_CODE'].to_csv(f'{CHECKPOINTS}/TXP_row_codes.csv', index=False)
    elif data_type == 'WL':
        pd.DataFrame(columns_to_drop, columns=['drop']).to_csv(f'{CHECKPOINTS}/WL_columns_to_drop.csv', index=False)
        df['WL_ID_CODE'].to_csv(f'{CHECKPOINTS}/WL_row_codes.csv', index=False)
...
def print_year_dist(df):
    ...
    plt.savefig(f"{IMAGES}/year_mortality.png", dpi=300)
...
def read_donornet_and_merge(df, file_path, time_name):
    ...
    merged_df['date_dif'] = (merged_df['TX_DATE'] - merged_df['donor_date']).abs()
    ...
    merged_df = merged_df.sort_values(by=['non_missing_count', 'date_dif', time_name], ascending=[False, True, False])

    # Remove duplicate DONOR_ID entries, keeping the first occurrence
    final_df = merged_df.drop_duplicates(subset=['DONOR_ID'])
    drop_col = ['TX_DATE', 'non_missing_count', 'date_dif', time_name, 'donor_date']
...
def fill_missing_donors_from_deceased(
    ...
    time_col,
    offer_time_col,
    deceased_path=f"{DATA}/thoracic_data/deceased_donor_data.csv",
):
...
def test_proportional_hazards_assumption(cph, data, id_col='WL_ID_CODE',
    ...
    os.makedirs(f'{RESULTS}/WL', exist_ok=True)
...
def evaluate(model_name, task_name, benchmark_choice, metrics, y_true, y_pred,
    ...
    if save_plots:
        ...
        plt.savefig(f'{IMAGES}/{{}}/ROC Curve {{}} & {{}}.png'.format(task_name, model_name, benchmark_choice))
        plt.close()
